<!DOCTYPE html>
<html lang="pt-br">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Sobre</title>
    <link rel="stylesheet" href="sobre.css">
</head>

<body>
    <div><a href="produtos.php"><button>Voltar</button></a></div>

    <h1>sobre nós</h1>
    <div class="membros">
        <div class="bio">
            <img src="fotos_membros/andreiperfil.jpeg" width="270px" height="270px" alt="perfildoandrei">
            <h2>Andrei</h2>
            <p>Desenvolvedor Back-end, responsável pela <br>inteligência e lógica do site.</p>
        </div>

        <div class="bio">
            <img src="fotos_membros/carlosperfil.jpeg" alt="perfildocarlos" width="270px" height="270px">
            <h2>Carlos </h2>
            <p>Desenvolvedor Front-end, focado na interface<br> e interação do usuário.</p>
        </div>

        <div class="bio">
            <img src="fotos_membros/mariaperfil.jpeg" alt="perfildamaria" width="270px" height="270px">
            <h2>Maria Lara</h2>
            <p> Web Designer, cuidando da identidade visual <br>e do layout inicial.</p>
        </div>

        <div class="bio">
            <img src="fotos_membros/sofiperfil.jpeg" alt="perfildasofia" width="270px" height="270px">
            <h2>Sofia Hage</h2>
            <p> Líder da equipe, responsável pela gestão<br> e organização do projeto.</p>
        </div>

        <div class="bio">
            <img src="fotos_membros/larissaperfil.jpeg" alt="perfilarissa" width="270px" height="270px">
            <h2>Larissa</h2>
            <p>Quality Assurance (QA)<br> e Banco de Dados, responsável<br> pelos testes e pela segurança das
                informações.</p>
        </div>


    </div>

    <div class="cabecalho">
        <p>INSTITUTO FEDERAL DE EDUCAÇÃO, CIÊNCIA E TECNOLOGIA DE RORAIMA
            <br> Cbv
            <br>Curso Técnico em Informática
            <br> Série: 2º Ano
        </p>
    </div>
</body>

</html>